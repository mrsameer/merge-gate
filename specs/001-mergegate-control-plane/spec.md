...
An operator enters a plain-language coding objective against a real repository. The system turns that objective into a set of measurable acceptance criteria (a "contract"), the operator reviews and approves the contract before any code is written, an autonomous coding agent proposes changes in an isolated workspace, and a separate deterministic engine — never the coding agent — computes a pass/fail verdict. On success the operator approves a final gate and the change is presented as a mergeable branch.
...
### User Story 7 - Visual control plane and human control (Priority: P3)

The workflow is presented on a canvas of typed nodes (Input, Agent, Command, Validator, Decision, Human Gate, Success, Stop), each showing name, type, status, attempt number, and latest result. An inspector shows only the selected node's settings. The operator can pause, resume, and stop a run, and the workflow can be exported and imported as configuration (which also serves as the no-hardcoding proof that graph paths are data, not code).
...
**Independent Test**: Load a workflow, run it, and watch node statuses/attempt counters update live; select a node and confirm the inspector reflects only that node; export the workflow to configuration and re-import it to reproduce the same graph.

**Acceptance Scenarios**:

1. **Given** a workflow, **When** it runs, **Then** each node visibly reflects its status, attempt number, and latest result as the run progresses.
2. **Given** a selected node, **When** the operator opens the inspector, **Then** only that node's settings are shown and editable.
3. **Given** a running workflow, **When** the operator pauses, resumes, or stops, **Then** the run honors the command and reflects the new state.
4. **Given** a workflow, **When** it is exported to configuration and re-imported, **Then** the same graph and node settings are reconstructed.
...
- **FR-026**: System MUST present the workflow as a canvas of typed nodes (Input, Agent, Command, Validator, Decision, Human Gate, Success, Stop), each showing name, type, status, attempt number, and latest result.
- **FR-027**: System MUST provide an inspector that shows and edits only the currently selected node's settings.
- **FR-028**: System MUST support exporting and importing the workflow as human-readable configuration, and the run behavior MUST follow that configuration (paths are data, not hardcoded).

**Configurable provider**

- **FR-029**: System MUST access the coding harness through a provider adapter interface, with the provider and model family selected via configuration rather than hardcoded.
- **FR-030**: System MUST allow substituting a different compatible provider without changing the workflow definition.

### Key Entities *(include if feature involves data)*

- **Workflow**: The typed-node graph defining the run's stages and the pass/fail routing between them; exportable/importable as configuration.
...
- **SC-010**: The full end-to-end story — objective → contract → run → first real failure → recovery → success → replay → clarification — is demonstrable within a 6-minute walkthrough.
